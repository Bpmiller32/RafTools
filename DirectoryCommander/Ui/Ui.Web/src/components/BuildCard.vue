...
<template>
  <div
    class="flex-grow max-w-md truncate bg-white rounded-lg shadow divide-y divide-gray-200"
  >
    <div class="flex items-center justify-between p-6">
      <div class="mr-4">
        ...
        <select
          ...
          class="border mt-2 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
        >
          <option v-for="(dir, index) in availableBuilds" :key="index">
            {{ dir[1] }}
          </option>
        </select>
      </div>
    ...
    <div class="flex justify-between">
      <div class="flex flex-grow mx-3 my-8 p-0.5 bg-gray-200 rounded-full">
        <p
          class="bg-indigo-500 p-0.5 text-xs font-medium text-white text-center leading-none rounded-full"
          :style="{ width: `${props.builder.progress}%` }"
        >
          {{ props.builder.progress }}%
        </p>
      </div>
...
function SetStyle(status) {
  if (status === "Ready") {
    ...
    statusBadgeStyle.value =
      "ml-3 px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full";
    buttonStyle.value =
      "flex items-center px-3 mr-6 my-4 rounded-md bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500";
  }
  if (status === "In Progress") {
    statusBadge.value = "In Progress";
    statusBadgeStyle.value =
      "ml-3 px-2 py-0.5 text-yellow-800 text-xs font-medium bg-yellow-100 rounded-full";
    buttonStyle.value =
      "flex items-center px-3 mr-6 my-4 rounded-md bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 cursor-not-allowed opacity-50";
  }
  if (status === "Error") {
    statusBadge.value = "Error";
    statusBadgeStyle.value =
      "ml-3 px-2 py-0.5 text-red-800 text-xs font-medium bg-red-100 rounded-full";
    buttonStyle.value =
      "flex items-center px-3 mr-6 my-4 rounded-md bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 cursor-not-allowed opacity-50";
  }
}
...
function BuildDirectory() {
  ...
  axios
    .post(`https://localhost:5001/api/Builder`, body)
    .then((response) => {
